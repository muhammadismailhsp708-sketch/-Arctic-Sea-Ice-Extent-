...
def load_data(filepath: str) -> pd.DataFrame:
    ...
    # If given path doesn't exist, try root directory (for Streamlit Cloud)
    if not os.path.exists(filepath):
        alt_path = os.path.join(os.path.dirname(os.path.dirname(filepath)), "arctic_ice_extent.csv")
        if os.path.exists(alt_path):
            filepath = alt_path
        else:
            # Try same directory as this file
            filepath = os.path.join(os.path.dirname(__file__), "arctic_ice_extent.csv")
